...
export default function ContactForm({ theme = 'frontend' }: ContactFormProps) {
  ...
  const focusBorder = isFrontend
    ? 'focus:border-[rgba(255,42,69,0.5)]'
    : 'focus:border-[rgba(31,224,208,0.5)]';
  const submitStyle = isFrontend
    ? 'bg-frontend text-[#0a0a0c]'
    : 'bg-cgi text-cgi-text-on';

  const fieldClass = `bg-bg-elevated border border-border rounded-[10px] px-4 py-3.5 text-text font-sans text-[15px] transition-colors duration-200 resize-y outline-none ${focusBorder}`;
  ...
  return (
    <form className="flex flex-col gap-5 max-w-[480px]" onSubmit={handleSubmit}>
      ...
      <button
        type="submit"
        className={`self-start border-none cursor-pointer px-7 py-3.5 rounded-full font-mono text-xs tracking-[1.5px] transition-[transform,opacity] duration-200 hover:-translate-y-0.5 ${submitStyle}`}
      >
        Send message →
      </button>
    </form>
